...
import { registerKeybind } from "./modules/venbind.ts";
import { unregisterKeybind } from "./modules/venbind.ts";
import { preregisterKeybinds } from "./modules/venbind.ts";
import { shouldPreregister } from "./modules/venbind.ts";
...
ipcMain.handle("venbind:registerKeybind", (event, shortcut, id) => { return registerKeybind(shortcut, id); });
ipcMain.handle("venbind:unregisterKeybind", (event, id) => { return unregisterKeybind(id); });
ipcMain.handle("venbind:preregisterKeybinds", (event, actions) => { return preregisterKeybinds(actions); });
ipcMain.on("venbind:shouldPreregister", (event, ) => { event.returnValue = shouldPreregister(); });
