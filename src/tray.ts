...
export async function createTray() {
    ...
    const contextMenu = Menu.buildFromTemplate([
        ...
        // --- NEW SETTING HERE ---
        {
            label: "Use Privacy User-Agent",
            type: "checkbox",
            checked: getConfig("usePrivacyUserAgent"),
            click: (item) => {
                setConfig("usePrivacyUserAgent", item.checked);
                console.log(`[Tray] Privacy User-Agent toggled to: ${item.checked}`);
                // Optional: If you want it to apply immediately without restart, uncomment below:
                // mainWindow.reload(); 
            },
        },
        {
