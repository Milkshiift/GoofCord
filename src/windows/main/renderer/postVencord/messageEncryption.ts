...
const EncryptionToggle = () => {
    ...
    return Common.React.createElement(
        Vencord.Api.ChatButtons.ChatBarButton,
        // @ts-expect-error
        {
...
export function initMessageEncryption() {
    ...
    // @ts-expect-error
    Vencord.Api.ChatButtons.addChatBarButton("messageEncryption", EncryptionToggle, UnlockIcon);

    Vencord.Api.MessageEvents.addMessagePreSendListener((_, msg) => {
        if (encryptionEnabled && msg.content) {
            msg.content = GoofCord.encryptMessage(msg.content);
        }
    });

    Vencord.Api.MessageEvents.addMessagePreEditListener((_, __, msg) => {
        if (encryptionEnabled && msg.content) {
            msg.content = GoofCord.encryptMessage(msg.content);
        }
    });
