...
function makeUniqueAlias(importPath: string, funcName: string): string {
	const sanitizedPath = importPath.replace(/^\.\//, "").replace(/[^a-zA-Z0-9$]/g, "_");

	return `${sanitizedPath}_${funcName}`;
}

// Code Generation

function generateHandler(channelName: string, aliasName: string, parameters: string, isAsync: boolean, type: "handle" | "on"): string {
	const prefix = type === "handle" ? "ipcMain.handle" : "ipcMain.on";
	const asyncKw = isAsync ? "async " : "";
	const awaitKw = isAsync ? "await " : "";

	const body = type === "handle" ? `return ${awaitKw}${aliasName}(${parameters});` : `event.returnValue = ${awaitKw}${aliasName}(${parameters});`;

	const params = parameters ? `, ${parameters}` : "";
	return `${prefix}("${channelName}", ${asyncKw}(event${params}) => { ${body} });`;
}

function generateHandlerFile(functions: IpcFunctionInfo[], collectedImports: Map<string, Map<string, string>>, sortedPaths: string[]): string {
	...
	for (const moduleSpecifier of sortedPaths) {
		const importMap = collectedImports.get(moduleSpecifier);
		if (!importMap) {
			throw new Error(`Imports for ${moduleSpecifier} unexpectedly missing`);
		}

		const importParts: string[] = [];
		const sortedEntries = Array.from(importMap.entries()).sort((a, b) => a[0].localeCompare(b[0]));

		for (const [originalName, aliasName] of sortedEntries) {
			importParts.push(`${originalName} as ${aliasName}`);
		}

		lines.push(`import { ${importParts.join(", ")} } from "${moduleSpecifier}";`);
	}
	...
	for (const func of functions) {
		const handler = generateHandler(func.channelName, func.aliasName, func.parameters, func.isAsync, func.type);
		lines.push(`  ${handler}`);
	}
...
function generateTypesFile(functions: IpcFunctionInfo[], registryHandlers: RegistryHandler[], collectedImports: Map<string, Map<string, string>>, sortedPaths: string[]): string {
	...
	if (sortedPaths.length > 0) {
		for (const moduleSpecifier of sortedPaths) {
			...
			lines.push(`import type { ${importParts.join(", ")} } from "${moduleSpecifier}";`);
		}
		lines.push("");
	}
	...
	for (const func of handleFunctions) {
		// Use typeof aliasName
		lines.push(`  "${func.channelName}": typeof ${func.aliasName};`);
	}
	lines.push("}", "");
...
function extractIpcFunctions(sourceFile: ts.SourceFile, ipcGenDir: string): { functions: IpcFunctionInfo[]; imports: Map<string, string> } {
	const functions: IpcFunctionInfo[] = [];
	const imports = new Map<string, string>();

	const filename = path.basename(sourceFile.fileName, path.extname(sourceFile.fileName)).replace(/\.(main|preload)$/, "");
	...
	ts.forEachChild(sourceFile, (node) => {
		...
		const aliasName = makeUniqueAlias(relativeImportPath, funcName);
		...
		imports.set(funcName, aliasName);
	});
...
export async function genIpcHandlers(config: Partial<GeneratorConfig> = {}) {
	...
	const collectedImports = new Map<string, Map<string, string>>();
